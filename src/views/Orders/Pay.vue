<template>
  <div class="pay">
    ...
    <div class="main" v-if="orderInfo">
      <div class="tip">
        <p class="countsown-ttle">支付剩余时间</p>
        <h3 class="countdown-text">{{ countDown }}</h3>
      </div>
      <section class="home">
        <ul class="list infolist">
          <li>
            <span class="order_name">{{ orderInfo.shopInfo.name }} </span>
      ...
      <button :disabled="timeOut" @click="pay" class="btn-submit">
        确认支付
      </button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    pay() {
      const data = {
        body: "test",
        out_trade_no: new Date().getTime().toSting(),
        total_fee: 1,
      };
      //请求
      fetch("/javascript", {
