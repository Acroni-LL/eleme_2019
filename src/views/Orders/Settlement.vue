<template>
  <div class="settlement">
    <Header :isLeft="true" title="确定订单" />
    <div class="view-body" v-if="orderInfo">
      <div class="">
        <!-- 收货地址 -->
        <section class="cart-address">
          <p class="title">
            订单配送至<span class="address-tag" v-if="userInfo && userInfo.tag"
              >{{ userInfo.tag }}
            </span>
          </p>
          <p class="address-detail">
            ...
            <span v-else>
              ...
              ></span>
              <span v-else @click="addAddress">新政收获地址</span>
              <i class="fa fa-angle-right"></i>
            </span>
          </p>
          <h2 v-if="userInfo" class="addresss-name">
        ...
        <section class="checkout-section">
          <CartItem
            @click="showTabware"
            title="餐具分数"
            :subHead="remarkInfo.tableware || '为悬着'"
          />
          <CartItem
  ...
  methods: {
    ...
    getData() {
      this.$axios(`/api/user/user_info/${localStorage.ele_login}`).then(
        (res) => {
          if (res.data.myAdrress.length > 0) {
