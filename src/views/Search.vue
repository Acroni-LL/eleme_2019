<template>
  <div class="search">
    ...
    <div class="container" v-if="showShop">
      ...
      >
        <IndexShop
          v-if="(item, index) in restaurants"
          :key="index"
...
export default {
  ...
  methods: {
    keyWordChange() {
      ...
      this.$axios(`/api/profile/typehead/${this.key_word}`)
        .then((res) => {
    ...
    searchHandler() {
      ...
      if (
        this.result &&
        (this.result.restaurants.length > 0 || tihs.result.words.length)
      ) {
        // console.log("有内容");
        this.shopItemClick();
    ...
    loadMore() {
      this.page++;
      this.$axios
        .post(`/api/peofile/restaurants/${this.page}/${this.size}`, this.data)
        .then((res) => {
